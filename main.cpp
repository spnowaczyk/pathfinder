...
bool detectCollision(int direction, int x, int y) {
    ...
    switch (direction) {
        case TOP:
            std::cout << "collider id: " << topCollider << "| if \"1\" collider detected: " << colliders[topCollider] << std::endl;
            return colliders[topCollider];
            break;
        case RIGHT:
            std::cout << "collider id: " << topCollider + 2 + (2 * heigth) << "| if \"1\" collider detected: " << colliders[topCollider + 2 + (2 * heigth)] << std::endl;
            return colliders[topCollider + 2 + (2 * heigth)];
            break;
        case DOWN:
            std::cout << "collider id: " << topCollider + 2 << "| if \"1\" collider detected: " << colliders[topCollider + 2] << std::endl;
            return colliders[topCollider + 2];
            break;
        case LEFT:
            std::cout << "collider id: " << topCollider + 1 << "| if \"1\" collider detected: " << colliders[topCollider + 1] << std::endl;
    ...
    for (int c = 0; c < 2000; c++) {
        ...
        for (auto i: generations.back()) {
            for (int j = 0; j < 4; j++) {
                ...
                std::cout << "since the old position was: " << i->posX << "|" << i->posY << " and modif was: "
                          << directionsX[j] << "|" << directionsY[j] << " new position is " << newPosX << "|" << newPosY
                          << std::endl;
                ...
                for (auto j: generations) {
                    for (auto k: j) {
                        if (newPosX == k->posX && newPosY == k->posY) visitedByOtherStepOtherGeneration = true;
                    }
                }
                ...
                if (!detectCollision(j, i->posX, i->posY) && !visitedByOtherStepOtherGeneration &&
                    ...
                    tempGen.push_back(new Step(newPosX, newPosY, i));
                    std::cout << ">>>>>>>>>coord pushed " << tempGen.back()->posX << "|" << tempGen.back()->posY
                              << std::endl;
                    if (tempGen.back()->posX == finishX && tempGen.back()->posY == finishY) {
>>>>>>> Stashed changes
            ...
            std::cout << generations.back().back()->posX << "|" << generations.back().back()->posY << std::endl;
            if (correctPath != nullptr) break;
        }
...
>>>>>>> Stashed changes
    ...
    int main() {
        ...
        deque = findWay(0, 0, 3, 4);

        for (auto i: deque) {
            std::cout << "X: " << i.first << " Y: " << i.second << std::endl;
        }

        detectCollision(LEFT, 0, 0);
